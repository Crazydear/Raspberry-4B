...
**此目录包含设备树覆盖。设备树使用单个内核即可支持许多硬件配置，而无需显式加载或将内核模块列入黑名单。请注意，这不是“纯”设备树配置（参见MACH_BCM2835）-某些板载设备仍通过板支持代码进行配置，但其目的是最终达到该目标。**
...
**在树莓派上，设备树的使用由```/boot/config.txt```控制。默认情况下，树莓派内核在启用设备树的情况下启动。您可以通过添加以下命令到你的```config.txt```来完全禁用设备树的使用（暂时），这将导致您的Pi在重启后恢复为旧的工作方式。**
...
**在`/boot`中，您将找到每个基本平台的`.dtb`。本节描述了树莓派板上的硬件组成部分。加载程序（`start.elf`及其兄弟姐妹）通过名称选择适用于平台的.dtb文件，并将其读入内存。此时，所有可选接口（i2c，i2s，spi）均已禁用，但可以使用设备树参数启用它们：**
...
**`raspi-config`是实用程序的“高级选项”部分可以启用和禁用设备树的使用，以及切换I2C和SPI接口。请注意，如果出于某种原因您应该推迟加载，则既可以启用接口也可以将驱动程序列入黑名单。**
...
**除了描述硬件外，设备树还提供了足够的信息以允许定位和加载合适的驱动程序模块，从而不加载不需要的模块。因此，可以从`/etc/modules`和`/etc/modprobe.d/raspi-blacklist.conf`中可以删除（或注释掉）其内容。**

四、Using Overlays（使用Overlays）
==============
...
**使用`dtoverlay`中的`config.txt`的设置加载Overlays。例如，考虑I2C实时时钟驱动程序。在预设备树的环境下，首先加载I2C接口和RTC设备的驱动程序，可以通过将包含设备标识符和I2C地址的魔法字符串写入`/sys/class/i2c-adapter`的特殊文件中来加载这些文件-如下所示：**
...
**这将导致文件`/boot/overlays/i2c-rtc.dtbo`被加载，并将描述DS1307 I2C设备的“节点”添加到树莓派的设备树中。默认情况下，它使用地址`0x68`，但是可以使用附加的DT参数对其进行修改：**
...
**尽管某些参数是必需的，但参数通常具有默认值。有关参数及其默认值的说明，请参见下面的overlays列表。**

五、Making new Overlays based on existing Overlays（在现有Overlays生成新的Overlays）
==============================================

> Recent overlays have been designed in a more general way, so that they can be adapted to hardware by changing their parameters. When you have additional hardware with more than one device of a kind, you end up using the same overlay
> multiple times with other parameters, e.g.

**最近的Overlays已经以更通用的方式进行了设计，因此可以通过更改其参数将其应用于硬件。当您拥有带有一种以上设备的其他硬件时，最终会多次使用相同的Overlay以及其他参数，例如：**

    # 2 CAN FD interfaces on spi but with different pins  spi上的CAN FD接口，但引脚不同
    dtoverlay=mcp251xfd,spi0-0,interrupt=25
    dtoverlay=mcp251xfd,spi0-1,interrupt=24
    
    # a realtime clock on i2c	i2c上的实时时钟
...
**尽管此方法确实有效，但它需要有关硬件设计的知识。与传统方式一样，通过提供单个overlay来简化最终用户的工作更为可行。**

> A new overlay can be generated by using ovmerge utility. [LINK](https://github.com/raspberrypi/utils/blob/master/ovmerge/ovmerge)

**新的overlay可以使用`ovmerge utility`程序生成。**[链接](https://github.com/raspberrypi/utils/blob/master/ovmerge/ovmerge)

> To generate an overlay for the above configuration we pass the configuration to ovmerge and add the -c flag.

**要为上述配置生成overlay，我们将配置传递给ovmerge并添加-c标志。**

    ovmerge -c mcp251xfd-overlay.dts,spi0-0,interrupt=25 mcp251xfd-overlay.dts,spi0-1,interrupt=24 i2c-rtc-overlay.dts,pcf85063 >> merged-overlay.dts
...
**-c选项将上面的命令作为注释写入到overlay中，作为该overlay的生成方式和生成方式的标记。编译overlay后，可以将其加载到一行中。**
...
六、The Overlay and Parameter Reference(Overlay及其参数参考)
===================================
...
Params:
        ...
        eth_led0                Set mode of LED0 - amber on Pi3B+ (default "1"),green on Pi4 (default "0").
        			# 设置LED0的模式-Pi3B +上为琥珀色（默认为“1”），Pi4上为绿色（默认为“0”）。
                                ...
                                Pi4
                                0=Speed/Activity         1=Speed 	2=Flash activity
                                3=FDX				4=Off      		5=On	 6=Alt                    
        ...
        act_led_gpio            Set which GPIO to use for the activity LED (in case you want to connect it to an external device)
        			# 设置用于活动LED的GPIO（如果要将其连接到外部设备）
                                ...
                                N.B. For Pi 3B, 3B+, 3A+ and 4B, use the act-led overlay.
                                # N.B。对于Pi 3B,3B+,3A+和4B，请使用act-led overlay。
